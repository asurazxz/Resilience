...
import { currentMonday, isMonday } from "../lib/date";
...
function Shell({ children }: { children: React.ReactNode }) {
  const nav = [["/", "Overview"], ["/entries", "Weekly entries"], ["/import", "Import CSV"], ["/settings", "Assumptions"]];
...
function WeekEditor() {
  const { weekStart = currentMonday() } = useParams();
  const { data, saveWeek } = useFoundation();
  const navigate = useNavigate();
  const existing = data.weeklyEntries.find((item) => item.weekStart === weekStart);
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      const snapshots: InputSnapshot[] = [...data.recurringWorkCosts.map((item) => ({ id: crypto.randomUUID(), sourceId: item.id, inputKind: "recurring_work_cost" as const, category: item.category, label: item.label, amountCents: item.amountCents, cadence: item.cadence })), ...data.essentialExpenses.map((item) => ({ id: crypto.randomUUID(), sourceId: item.id, inputKind: "essential_expense" as const, category: item.category, label: item.label, amountCents: item.amountCents, cadence: item.cadence }))];
      await saveWeek({ id: existing?.id ?? crypto.randomUUID(), weekStart: draft.weekStart, hadNoIncome: draft.noIncome, emergencySavingsCents: parseMoneyToCents(draft.emergency), status: "confirmed", revision: existing?.revision ?? 0, earnings: earningRows.map((row) => ({ id: row.id, platformCode: row.category as EarningItem["platformCode"], platformLabel: row.category === "other" ? row.label : null, amountCents: row.amountCents })), variableCosts: variableRows.map((row) => ({ id: row.id, category: row.category as VariableCostItem["category"], label: row.label, amountCents: row.amountCents })), inputSnapshots: snapshots });
      navigate("/entries");
